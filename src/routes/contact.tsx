...
export const Route = createFileRoute("/contact")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Get in touch with TC Real Estates — verified property deals in Ahmedabad. Call, WhatsApp or enquire online." },
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="max-w-7xl mx-auto px-6 py-12 grid md:grid-cols-2 gap-10">
        <div className="space-y-5">
          <ContactCard icon={Phone} title="Call us" lines={[PHONE_NUMBER]} href={`tel:${PHONE_NUMBER.replace(/\s/g, "")}`} action="Call now" />
          <ContactCard icon={MessageCircle} title="WhatsApp" lines={["Chat with our team"]} href={waLink()} action="Open chat" external />
          <ContactCard icon={Mail} title="Email" lines={["info@tcrealestates.in"]} href="mailto:info@tcrealestates.in" action="Send email" />
          <ContactCard icon={MapPin} title="Office" lines={["Ahmedabad, Gujarat, India"]} />
          <ContactCard icon={Clock} title="Working hours" lines={["Mon - Sat: 9:00 AM - 8:00 PM"]} />
        </div>
        <div>
...
function ContactCard({
  icon: Icon, title, lines, href, action, external,
}: {
  icon: any; title: string; lines: string[]; href?: string; action?: string; external?: boolean;
}) {
  return (
