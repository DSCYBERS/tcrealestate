...
import {
  LayoutDashboard, Home as HomeIcon, Building2, Users, MapPin, MessageSquare, FileText, Settings,
  LogOut, Loader2, Menu,
} from "lucide-react";
...
function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-muted/30 flex">
      {/* Sidebar */}
      <aside className={`${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:sticky inset-y-0 left-0 z-40 w-64 bg-brand-dark text-white flex flex-col transition-transform top-0 h-screen`}>
        <div className="p-6 border-b border-white/10">
          <Link to="/" className="flex items-center gap-2.5">
            <div className="bg-brand-yellow text-brand-dark font-extrabold text-lg w-10 h-10 flex items-center justify-center rounded-sm">TC</div>
            <div>
        ...
        <div className="p-3 border-t border-white/10 space-y-2">
          <Link to="/" className="flex items-center gap-3 px-3 py-2 rounded-md text-sm text-white/80 hover:bg-white/10">
            <HomeIcon className="w-4 h-4" /> View site
          </Link>
          <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm text-white/80 hover:bg-white/10">
      ...
      <main className="flex-1 min-w-0">
        <div className="lg:hidden sticky top-0 z-20 bg-background border-b border-border px-4 py-3 flex items-center justify-between">
          <button onClick={() => setMobileOpen(true)} className="p-2 rounded-md border border-border">
