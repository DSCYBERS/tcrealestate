  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <header className="flex items-center justify-between mb-6">
        <div>
          <Link to="/admin" className="text-sm text-primary hover:underline">← Back</Link>
          <h1 className="text-2xl font-bold mt-2">Property Management</h1>
        </div>
        <button onClick={() => setEditing({})} className="px-4 py-2 bg-primary text-white rounded">Add Property</button>
      </header>
        ...
        {list.map((p) => (
          <div key={p.id} className="bg-white p-4 rounded shadow flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-muted">{p.location} — {p.totalPrice}</div>
            </div>
            <div className="flex items-center gap-2">
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={!!p.featured} onChange={(e) => toggleFeatured(p.id, e.target.checked)} />
                <span className="text-sm">Featured</span>
              </label>
              <button onClick={() => setEditing(p)} className="px-3 py-1 border rounded">Edit</button>
              <button onClick={() => onDelete(p.id)} className="px-3 py-1 border rounded">Delete</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {editing && (
        <form onSubmit={onSave} className="mt-6 bg-white p-4 rounded shadow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <input name="name" defaultValue={editing.name || ''} placeholder="Name" className="border px-3 py-2 rounded" />
            <input name="location" defaultValue={editing.location || ''} placeholder="Location" className="border px-3 py-2 rounded" />
            <input name="totalPrice" defaultValue={editing.totalPrice || ''} placeholder="Total Price" className="border px-3 py-2 rounded" />
          </div>
