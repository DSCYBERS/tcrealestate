...
type SiteContent = Record<string, never>;
    ...
    <div className="p-6 lg:p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Edit hero, CTAs, contact info, and homepage SEO.
          </p>
        </div>
        <button
          onClick={saveAll}
          disabled={busy}
          className="inline-flex items-center gap-2 bg-primary text-primary-foreground font-bold px-5 py-2.5 rounded-md disabled:opacity-50"
        >
          {busy ? (
      ...
      <Section title="Hero Section">
        ...
        <K
          ...
          v={hero.highlight}
          o={(v) => setHero({ ...hero, highlight: v })}
        />
        <K
          label="Subtitle"
          v={hero.subtitle}
          o={(v) => setHero({ ...hero, subtitle: v })}
          textarea
        />
        <div className="grid sm:grid-cols-2 gap-3">
          <K
            ...
            v={hero.cta_primary}
            o={(v) => setHero({ ...hero, cta_primary: v })}
          />
          <K
            label="Secondary CTA text"
            v={hero.cta_secondary}
            o={(v) => setHero({ ...hero, cta_secondary: v })}
          />
        </div>
      </Section>
      ...
      <Section title="Contact Details">
        <div className="grid sm:grid-cols-2 gap-3">
          <K
            label="Phone (display)"
            v={contact.phone}
            o={(v) => setContact({ ...contact, phone: v })}
          />
          <K
            label="WhatsApp (digits only)"
            v={contact.whatsapp}
            o={(v) => setContact({ ...contact, whatsapp: v })}
          />
          <K label="Email" v={contact.email} o={(v) => setContact({ ...contact, email: v })} />
          <K
            label="Address"
            v={contact.address}
            o={(v) => setContact({ ...contact, address: v })}
          />
        </div>
      </Section>
      ...
      <Section title="Homepage SEO">
        <K
          label="Page Title (under 60 chars)"
          v={seo.title}
          o={(v) => setSeo({ ...seo, title: v })}
        />
        <K
          label="Meta Description (under 160 chars)"
          v={seo.description}
          o={(v) => setSeo({ ...seo, description: v })}
          textarea
        />
