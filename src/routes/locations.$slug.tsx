...
export const Route = createFileRoute("/locations/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const desc = l ? `${l.tagline}. Starting at ₹${l.startingPrice}. Verified plots in ${l.name}, Ahmedabad.` : "";
    return {
      meta: [
        { title },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-6 py-20">
        ...
        <Link to="/properties" className="mt-6 bg-primary text-primary-foreground px-5 py-3 rounded-md text-sm font-bold">
          View all properties
        </Link>
      </div>
      <SiteFooter />
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[70vh] min-h-[480px] flex items-center">
        ...
        <div className="relative max-w-7xl mx-auto px-6 w-full text-white">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <a href={waLink(waMsg)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-6 py-3 rounded-md font-bold">
              <MessageCircle className="w-4 h-4" /> Enquire Now
            </a>
            <a href={`tel:${PHONE_NUMBER.replace(/\s/g, "")}`} className="inline-flex items-center gap-2 bg-white text-brand-dark px-6 py-3 rounded-md font-bold">
              <Phone className="w-4 h-4" /> Contact Expert
            </a>
          </div>
          <p className="mt-4 text-sm text-white/80">Starting at <span className="text-brand-yellow font-bold">₹{loc.startingPrice}</span></p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-14 grid lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-10">
          <div>
            <p className="text-xs tracking-[0.25em] text-primary font-bold">ABOUT {loc.name.toUpperCase()}</p>
            <h2 className="mt-2 text-2xl md:text-3xl font-extrabold">Why {loc.name} is a smart pick</h2>
        ...
        <div className="lg:sticky lg:top-24 h-fit">
          <LeadForm title={`GET ${loc.name.toUpperCase()} DEALS`} subtitle="BEFORE MARKET" defaultPropertyId={`location-${loc.slug}`} />
        </div>
      </section>
